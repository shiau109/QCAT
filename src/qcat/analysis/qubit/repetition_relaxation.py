...
class RelaxationRepetitionAnalysis(QCATAna):
    """
    ...
    def _start_analysis(self):
        ...
        self.statistic_result = xr.Dataset(
            {
                "T1": (dims, np.array(statistic_t1)),  # Variable with dimensions
            },
    ...
    def _plot_results(self):
        ...
        t1 = self.statistic_result["T1"].values

        raw_data = self.data.transpose()*1000
        median_ans, std_ans =np.median(t1), np.std(t1)
        ...
        ax1.hist(t1, bins='auto', density=False)
        ax1.axvline(median_ans,c='k',ls='--',lw='1')
        ax1.set_ylabel("Counts",fontsize=20)
        ax1.set_xlabel(f"T1 (us)", fontsize=20)
        ax1.set_title(f"T1 = {round(median_ans,1)} $\pm$ {round(std_ans,1)} us",fontsize=20)

        ax2 = fig.add_subplot(gs[:,0])

        c = ax2.pcolormesh(rep, time, raw_data, cmap='RdBu')
        fig.colorbar(c, ax=ax2, label='Contrast (mV)',location='bottom',)
        ax2.plot(rep, t1, c='green')
...
if __name__ == '__main__':
    ...
    for ro_name, data in dataset.data_vars.items():
        ...
        data.name = ro_name
        my_ana = RelaxationRepetitionAnalysis(data.sel(mixer="I"))
