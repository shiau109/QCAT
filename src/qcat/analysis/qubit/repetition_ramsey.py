...
class RamseyRepetitionAnalysis(QCATAna):
    """
    ...
    def _start_analysis(self):
        ...
        for i in range(repetition_times):
            ...
            fit_ramsey = FitDampedOscillation( self.data.isel(repetition=i).rename({"time": "x"}) )
        ...
        self.statistic_result = xr.Dataset(
            {
                "T2": (dims, np.array(statistic_t1)),  # Variable with dimensions
            },
    ...
    def _plot_results(self):
        ...
        t2 = self.statistic_result["T2"].values
        ...
        ax1.set_xlabel(f"T2 (us)", fontsize=20)
        ax1.set_title(f"T2 = {round(median_ans,1)} $\pm$ {round(std_ans,1)} us",fontsize=20)

        ax2 = fig.add_subplot(gs[:,0])

        c = ax2.pcolormesh(rep, time, raw_data, cmap='RdBu')
        fig.colorbar(c, ax=ax2, label='Contrast (mV)',location='bottom',)
        ax2.plot(rep, t2, c='green')
...
if __name__ == '__main__':
    import xarray as xr
    dataset = xr.open_dataset(r"d:\Data\ramseyT2_rep.nc")
    print(dataset)
    for ro_name, data in dataset.data_vars.items():
        data.attrs = dataset.attrs
        data.name = ro_name
        my_ana = RamseyRepetitionAnalysis(data.sel(mixer="I"))
