...
def fq_flux( x, offset, Ec, Ej_sum, d, period):

    quan_flux = volt2quantFlux( x, offset, period  )
    Ej_eff = effective_Ej( quan_flux, Ej_sum, d)
    return np.sqrt(8*Ec*Ej_eff)-Ec


def effective_Ej( quan_flux, Ej_sum, d ):
    return Ej_sum*np.sqrt(np.cos( np.pi*quan_flux )**2+d**2*np.sin( np.pi*quan_flux )**2)

def volt2quantFlux( volt, offset, period ):
    return (volt-offset)/period

def _fluxDepFreq_model():
    ...
    params['Ej_sum'].set(min=0.) 
    params['Ec'].set(min=0.) 
    params['d'].set(min=-1, max=1) # Frequency between 0 and 10 radians per second
    params['period'].set(min=0.)
    return model, params

def qubit_flux_spec_fitting( flux, data, period=None ):


    model, params = _fluxDepFreq_model()
    if period != None:
        params['period'].set( period )#, vary=False)


    result = model.fit(data, params, x=flux)
    return result
