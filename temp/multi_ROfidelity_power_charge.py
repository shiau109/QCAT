...
assign_mean = None# load_xarray_h5(r"d:\data\MIST\charge_ramsey_power_fidelity\41_500_1\#1420_LCH_const_charge_readout_power_20_010529\free_std5\fit_mean.nc")
...
for sq_data in qubit_datasets:
    ...
    std = merged_summary['std'].transpose('charge_volt', 'amp_prefactor').values
    fig3, ax3 = plt.subplots(figsize=(8, 6))
    im3 = ax3.imshow(std, aspect='auto', origin='lower',
                    extent=[amp_prefactor[0], amp_prefactor[-1], charge_volt[0], charge_volt[-1]],
                    cmap='plasma')
    ax3.set_xlabel('amp_prefactor')
    ax3.set_ylabel('charge_volt')
    ax3.set_title(f'Qubit {qubit_name} State {state}: std')
    fig3.colorbar(im3, ax=ax3, label='std')
    fig3.tight_layout()
    fig3.savefig(os.path.join(base_dir, f'std_2d_{qubit_name}_state{state}.png'))
    plt.close(fig3)
