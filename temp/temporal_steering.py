...
def analyze_temporal_steering_folder(base_dir):
    ...
    print(f"Total datasets loaded: {len(dataset_list)} in {base_dir}")

    # Build merged_ds and save it
    ds_list = []
    basis_list = []
    for ds, json_dict in dataset_list:
        basis_list.append(json_dict["data"]["parameters"]["model"]["readout_basis"])
        ds_list.append(ds)
    ...
    for sq_data in qubit_datasets:
        ...
        signal = sq_data['signal'].values  # shape: (readout_basis, idle_time)
        basis_map = {b: i for i, b in enumerate(bases)}
        Sx = 1- 2*signal[basis_map['X'], :]
        Sy = 1- 2*signal[basis_map['Y'], :]
        Sz = 1- 2*signal[basis_map['Z'], :]
        from qutip import Qobj, sigmax, sigmay, sigmaz, qeye
        density_matrices = []
        for x, y, z in zip(Sx, Sy, Sz):
            rho = (qeye(2) + x*sigmax() + y*sigmay() + z*sigmaz())
...
root_dir = r'D:\data\temporal_steering\v1_1k'
